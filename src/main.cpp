...
int main()
{
	...
	IrrlichtDevice* device = createDevice(driverType,
			core::dimension2d<u32>(screenW, screenH), 16, false, false, false, &receiver);
	...
	*/
	...
	while(device->run())
	{
		...
		core::vector3df nodePosition = player_node->getPosition();
		...
		*/
		...
		//	player_node->setFrameLoop(168, 188);
		//	player_node->setAnimationSpeed(32);
		//	player_node->setLoopMode(false);
		//}
		//else
		//{
		//	player_node->setFrameLoop(0, 80);
		//	player_node->setLoopMode(true);
		//}
		nodePosition += speed * dtime;

		player_node->setPosition(nodePosition);
		...
		player_node->setRotation(v3f(0, -1 * camera_yaw, 0));
		player_node->setPosition(nodePosition);
		v3f camera_position =
			player_node->getPosition() + v3f(0, BS*1.65, zoom_max) + zoom_direction * camera_zoom;
